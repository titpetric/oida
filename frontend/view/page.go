...
// NewPage builds the view model shared by every component: the snapshot to
// render, the page it is rendered as, and the filters the request asks for.
func NewPage(opts model.Options, snapshot model.Snapshot, name View, r *http.Request) Page {
	page := Page{
		Snapshot: snapshot,
		View:     name,
		Path:     opts.Path,
...
// NewLivePage builds the live view: traces in flight and completed traces merged
