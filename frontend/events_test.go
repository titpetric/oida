...
func TestEventStreamPushesLiveSection(t *testing.T) {
	...
	server := httptest.NewServer(HandlerFor(tracer))
	defer server.Close()
...
func TestEventStreamDisabled(t *testing.T) {
	...
	response := request(t, HandlerFor(tracer), oida.DefaultPath+"/live/events", nil)
	if response.Code != http.StatusNotFound {
		t.Fatalf("disabled stream returned %d, want 404", response.Code)
	}

	body := request(t, HandlerFor(tracer), oida.DefaultPath+"/live", nil).Body.String()
...
func TestLiveViewStreamOff(t *testing.T) {
	...
	streaming := request(t, HandlerFor(tracer), oida.DefaultPath+"/live", nil).Body.String()
	...
	static := request(t, HandlerFor(tracer), oida.DefaultPath+"/live?stream=off", nil).Body.String()
...
func TestLiveFeedHoldsRunningAndCompletedTraces(t *testing.T) {
	...
	body := request(t, HandlerFor(tracer), oida.DefaultPath+"/live?stream=off", nil).Body.String()
