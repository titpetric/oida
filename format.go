...
func sortedKeys(attributes Attributes) []string {
...
func keyList(attributes Attributes) string {
...
func shapeTitle(trace Trace) string {
	segments := Timeline(trace)
	if len(segments) == 0 {
		return durationText(trace.Duration)
	}

	shares := make(map[Kind]float64, len(segments))
	order := make([]Kind, 0, len(segments))
...
func matches(trace Trace, query string) bool {
	if query == "" {
		return true
	}
...
func slowest(traces []Trace) time.Duration {
...
func recent(traces []Trace, n int) []Trace {
	if n > 0 && len(traces) > n {
		return traces[:n]
	}
	return traces
...
// empty cell.
func remoteText(trace Trace) string {
...
func traceDot(trace Trace) string {
...
func hostDot(host HostStat) string {
	if host.Errors > 0 {
		return "dot bad"
	}
	return "dot ok"
...
func hostHealth(host HostStat) string {
	if host.Errors == 0 {
...
func stateClass(state State) string {
