...
// AssetURL links a file in the embedded public tree.
...
// on the page.
func (p Page) DurationShare(trace Trace) float64 {
...
func (p Page) Composition(trace Trace) []Segment {
...
func (p Page) KindShares() []Segment {
	shares := make(map[Kind]*Segment, len(p.Segments))
	order := make([]Kind, 0, len(p.Segments))
