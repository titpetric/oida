...
func hostRouter(t *testing.T, service, span string) vhost {
	...
	mux.HandleFunc("GET /work", func(w http.ResponseWriter, r *http.Request) {
		StartSpan(r.Context(), span, KindDatabase).End()
	...
	return vhost{tracer: tracer, handler: TracingMiddleware(opts)(mux)}
}
...
func dashboard(t *testing.T, router http.Handler, host string) string {
	...
	router.ServeHTTP(response, httptest.NewRequest(http.MethodGet, "http://"+host+DefaultPath+"/traces", nil))
